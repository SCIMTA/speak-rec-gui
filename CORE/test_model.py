...
def call_similar(x):
    no_batch = int(x.shape[0] / (
                num_neg + 2))  # each batch was consisted of 1 anchor ,1 positive , num_neg negative, so the number of batch
    similar = []
...
def eval_model(model, train_batch_size=c.BATCH_SIZE * c.TRIPLET_PER_BATCH, test_dir=c.TEST_DIR, check_partial=False,
    ...
    fm, tpr, acc, eer, threshold = evaluatebyeer(y_pred, y_true)
...
if __name__ == '__main__':
    ...
    fm, tpr, acc, eer, threshold = eval_model(model, check_partial=True)
    print(
        "f-measure = {0}, true positive rate = {1}, accuracy = {2}, equal error rate = {3},with correct threshold = {4}".format(
            fm, tpr, acc, eer, threshold))
