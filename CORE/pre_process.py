...
def read_audio(filename, sample_rate=SAMPLE_RATE):
    _audio, sr = librosa.load(filename, sr=sample_rate, mono=True)
    audio = VAD(_audio.flatten())
    start_sec, end_sec = c.TRUNCATE_SOUND_SECONDS
    start_frame = int(start_sec * SAMPLE_RATE)
    end_frame = int(end_sec * SAMPLE_RATE)

    if len(audio) < (end_frame - start_frame):
        k = int(((end_frame - start_frame) / len(audio)) + 5)
...
def prep(libri, out_dir=c.DATASET_DIR, name='0'):
    ...
    for i in range(len(libri)):
        ...
        if i % 100 == 0:
            print("task:{0} cost time per audio: {1:.3f}s No.:{2} File name:{3}".format(name, time() - orig_time, i,
                                                                                        filename))
...
def preprocess_and_save(wav_dir=c.WAV_DIR, out_dir=c.DATASET_DIR):
    orig_time = time()
    libri = data_catalog(wav_dir,
                         pattern='**/*.flac')  # '/Users/walle/PycharmProjects/Speech/coding/deep-speaker-master/audio/LibriSpeechSamples/train-clean-100/19'
...
def test():
    libri = data_catalog()
    filename = 'audio/LibriSpeechSamples/datatrain/hang/hang(1).wav'
    raw_audio = read_audio(filename)
    print(filename)
    feature = extract_features(raw_audio, target_sample_rate=SAMPLE_RATE)
    print(filename)
